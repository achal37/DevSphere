  ...
  return (
    <>
      <div className="navbar flex items-center justify-between px-[100px] h-[80px] bg-[#141414]">
        <div className="logo">
          <img className='w-[150px] cursor-pointer'
            src={logo}
            onClick={() => navigate('/')}
            alt="" />
        </div>
        <div className="links flex items-center gap-2">
          <button onClick={logout} className='btnBlue !bg-red-500 min-w-[10px] ml-2 hover:!bg-red-600'><IoLogOut className='text-2xl' /></button>
          <Avatar onClick={() => { toggleClass(".dropDownNavbar", "hidden") }} name={data ? data.name : ""} size="40" round="50%" className=' cursor-pointer ml-2' />
        </div>

        <div className='dropDownNavbar hidden absolute right-[60px] top-[80px] shadow-lg shadow-black/50 p-[10px] rounded-lg bg-[#1A1919] w-[150px]'>
